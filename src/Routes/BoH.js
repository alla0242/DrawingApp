...
const BoH = ({ width, height }) => {
            ...
            if (data.success) {
                ...
                setLatestImages(prevImages =>
                    prevImages.map(image =>
                        image.id === id ? { ...image, state: newState, lastChangeTime: new Date(), lastChangeSource: 'BoH' } : image
                    )
                );
    ...
    return (
        <div>
                ...
                <div>
                    {latestImages.map((image, index) => (
                        <div key={index} style={{margin: '20px 0', border: '1px solid #ccc', padding: '10px'}}>
                            ...
                            <button 
                                onClick={() => updateImageState(image.id.toString(), "Ready for Pickup")}
                                disabled={image.state === "Ready for Pickup"}
                            >
                                Mark as Ready for Pickup
                            </button>
                            <button 
                                onClick={() => updateImageState(image.id.toString(), "Order at table")}
            ...
            <button onClick={fetchLatestImages}>
                Refresh Images
            </button>
        </div>
    );
